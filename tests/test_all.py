...
def test_add_2(fresh_db):
    success, message = ls.add_book_to_catalog("", "Test Author","1234567890124", 5)
    assert success == False
    assert "title is required" in message.lower()

def test_add_3(fresh_db):
    success, message = ls.add_book_to_catalog("Testing Book", "", "1234567890125",5)
    assert success == False
    assert "author is required" in message.lower()

def test_add_4(fresh_db):
    success, message = ls.add_book_to_catalog("Testing Book", "Testing Author", "0",5)
    assert success == False
    assert "isbn must" in message.lower()

def test_add_5(fresh_db):
    success, message = ls.add_book_to_catalog("Testing a Book", "Testing A. Author","1234567890126", -1)
    assert success == False
...
def test_get_all_3(fresh_db):
    ls.add_book_to_catalog("New Book", "New Author", "1234567891111", 6)
...
def test_borrow_1(fresh_db):
    success, message = ls.borrow_book_by_patron("000000", 1)
    assert success == True
    assert "successfully" in message.lower()

def test_borrow_2(fresh_db):
    success, message = ls.borrow_book_by_patron("1", 1)
    assert success == False
    assert "invalid patron" in message.lower()

def test_borrow_3(fresh_db):
    success, message = ls.borrow_book_by_patron("000000", 3)
    assert success == False
    assert "not available" in message.lower()

def test_borrow_4(fresh_db):
    success, message = ls.borrow_book_by_patron("000000", 0)
    assert success == False
    assert "not exist" in message.lower()

def test_return_1(fresh_db):
    success, message = ls.return_book_by_patron("111", 4)
    assert success == False
    assert "invalid patron" in message.lower()
...
def test_return_3(fresh_db):
    ls.borrow_book_by_patron("000000", 1)
    success, message = ls.return_book_by_patron("000000", 1)
    assert success == True
    assert "successfully" in message.lower()

def test_return_4(fresh_db):
    success, message = ls.return_book_by_patron("000000", 304)
    assert success == False
...
def test_fees_1(fresh_db):
    ...
    """
    result = ls.calculate_late_fee_for_book("1280", 4)
...
def test_fees_2(fresh_db):
    ...
    """
    result = ls.calculate_late_fee_for_book("000000", 304)
    assert result['fee_amount'] == 0
...
def test_fees_3(fresh_db):
    ...
    ls.return_book_by_patron("000000", 1)

def test_fees_4(fresh_db):
    """
    This book is not available so should never be overdue.
    """
    result = ls.calculate_late_fee_for_book("000000", 3)
    assert result['fee_amount'] == 0
    assert result['days_overdue'] == 0
...
def test_search_1(fresh_db):
    result = ls.search_books_in_catalog("1984", "title")
...
def test_search_2(fresh_db):
    result = ls.search_books_in_catalog("George", "author")
    assert isinstance(result, list)

def test_search_3(fresh_db):
    result = ls.search_books_in_catalog("9780451524935", "isbn")
    assert len(result) == 1
    assert result[0]['isbn'] == "9780451524935"
    assert isinstance(result, list)

def test_search_4(fresh_db):
    result = ls.search_books_in_catalog("0p", "title")
    assert isinstance(result, list)
    assert result == []
...
def test_patron_1(fresh_db):
    ...
    result = ls.get_patron_status_report("0")
    assert isinstance(result, dict)
    for key in expected_keys:
        assert key in result

def test_patron_2(fresh_db):
    result = ls.get_patron_status_report("000111")
    assert isinstance(result, dict)
...
def test_patron_4(fresh_db):
    result = ls.get_patron_status_report("gol")
    assert isinstance(result, dict)
    assert "invalid patron" in result['status'].lower()