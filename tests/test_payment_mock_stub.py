...
def test_pay_late_fees_1(fresh_db, mocker, sample_book):
    mocker.patch('services.library_service.calculate_late_fees_for_book', return_value={'fee_amount': 5.0, 'days_overdue': 3, 'status': 'Overdue'})
...
def test_pay_late_fees_2(fresh_db, mocker, sample_book):
    mocker.patch('services.library_service.calculate_late_fees_for_book', return_value={'fee_amount': 7.5, 'days_overdue': 5, 'status': 'Overdue'})
    mocker.patch('services.library_service.get_book_by_id', return_value=sample_book)
...
def test_pay_late_fees_3(fresh_db, mocker, sample_book):
    mocker.patch('services.library_service.calculate_late_fees_for_book', return_value={'fee_amount': 10.0, 'days_overdue': 6, 'status': 'Overdue'})
    mocker.patch('services.library_service.get_book_by_id', return_value=sample_book)
    mock_gateway = Mock(spec=PaymentGateway)
...
def test_pay_late_fees_4(fresh_db, mocker, sample_book):
    mocker.patch('services.library_service.calculate_late_fees_for_book', return_value={'fee_amount': 0.0, 'days_overdue': 0, 'status': 'On time'})
    mocker.patch('services.library_service.get_book_by_id', return_value=sample_book)
    mock_gateway = Mock(spec=PaymentGateway)
